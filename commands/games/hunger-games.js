...
module.exports = class HungerGamesCommand extends Command {
      ...
      args: [
        {
          key: 'tributes',
          prompt:
            'Who should compete in the games? Up to 24 tributes can participate.',
          type: 'string',
          infinite: true,
          max: 20
        }
  ...
  async run(msg, { tributes }) {
    if (tributes.length < 2)
      return msg.say(`...${tributes[0]} wins, as they were the only tribute.`);
  ...
  makeLeaderboard(tributes, kills) {
    ...
    return tributes
      ...
      .map(tribute => {
        ...
        return `**${i}.** ${tribute} (${kills[tribute]} Kill${
          kills[tribute] === 1 ? '' : 's'
        })`;
      });
  }
